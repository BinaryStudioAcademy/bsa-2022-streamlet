import { AppRoutes, DataStatus, IconName } from 'common/enums/enums';
...
import { useAppDispatch, useAppSelector, useCallback, useEffect, useNavigate } from 'hooks/hooks';
import { errorCodes, StreamPermission } from 'shared/build';
...
const StudioHomeContainer: FC = () => {
  ...
  const { channel, stream, errorCode, error, status, streamPermission } = useAppSelector((state) => ({
  ...
  useEffect(() => {
    if (streamPermission.streamPermission !== StreamPermission.ALLOWED) {
      navigate(AppRoutes.ROOT, { replace: true });
    }
  }, [streamPermission.streamPermission, navigate]);
